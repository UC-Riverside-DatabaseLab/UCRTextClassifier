...
class TextCNN(object):
	...
	def __init__(
	  ...
	  embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0, word2vec=None, relu1=False, relu2=False):
		...
		with tf.device('/cpu:0'), tf.name_scope("embedding"):
			if word2vec != None:
				W = tf.Variable(
					tf.pack(np.array([len(word2vec.values()[0]) * [0]] + word2vec.values())),
					name="W")
				embedding_size = len(word2vec.values()[0])
		...
		if relu1:
			...
			with tf.name_scope("relu1"):
				wh1 = tf.Variable(tf.random_normal([num_filters_total, n_hidden1]), name="Wh")
				bh1 = tf.Variable(tf.random_normal([n_hidden1]), name="Bh")
				relulayer_1 = tf.nn.relu(tf.nn.xw_plus_b(self.h_drop, wh1, bh1))
		
			if relu2:
				# Add anothe hidden layer
				n_hidden2 = 128
				with tf.name_scope("relu2"):
					wh2 = tf.Variable(tf.random_normal([n_hidden1, n_hidden2]), name="Wh")
					bh2 = tf.Variable(tf.random_normal([n_hidden2]), name="Bh")
					relulayer_2 = tf.nn.relu(tf.nn.xw_plus_b(relulayer_1, wh2, bh2))
			else:
